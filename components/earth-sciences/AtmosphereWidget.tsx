      ...
      <div className="grid grid-cols-4 gap-2 mb-4 shrink-0">
        {[
          ...
          <button
            ...
            className={`py-2 px-1 rounded-xl text-[10px] md:text-xs font-black transition-all cursor-pointer border flex flex-col md:flex-row items-center justify-center gap-1.5 ${
              activeTab === item.tab 
      ...
      <div className={`flex-grow flex flex-col justify-between overflow-hidden min-h-0 ${isFullScreen ? "max-h-[72vh]" : "max-h-[580px]"}`}>
              ...
              <div className="relative w-full md:w-1/2 h-full min-h-[160px] bg-slate-900/60 border border-zinc-800 rounded-xl overflow-hidden flex items-center justify-center p-2">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <svg className="w-full h-full" viewBox="0 0 200 120">
                    {/* Incoming Sunbeam */}
                    <line x1="20" y1="20" x2="100" y2="100" stroke="#f59e0b" strokeWidth="2.5" strokeDasharray="3 3" className="animate-pulse" />
                    <text x="25" y="15" fill="#f59e0b" className="text-[7px] font-black">קרינת שמש (100%)</text>
                    ...
                    {surfaces[selectedSurface].albedo > 0.05 && (
                      <line 
                        ...
                        stroke="#fef08a" 
                        strokeWidth={1.5 + (surfaces[selectedSurface].albedo * 2.5)} 
                      />
                    )}
                    ...
                    <path 
                      ...
                      fill="none" 
                      stroke="#ef4444" 
                      strokeWidth={1 + (1 - surfaces[selectedSurface].albedo) * 4} 
                      className="animate-pulse"
                      strokeDasharray="2 2"
                    />

                    {/* Albedo text label */}
                    <text x="95" y="112" fill="#fff" textAnchor="middle" className="text-[8px] font-black">
                      {surfaces[selectedSurface].name} (אלבדו: {Math.round(surfaces[selectedSurface].albedo * 100)}%)
                    </text>
                    <text x="140" y="30" fill="#fef08a" className="text-[6.5px] font-bold">
                      החזרה: {Math.round(surfaces[selectedSurface].albedo * 100)}%
                    </text>
                    <text x="75" y="30" fill="#f87171" className="text-[6.5px] font-bold">
                      בליעה: {Math.round((1 - surfaces[selectedSurface].albedo) * 100)}%
                    </text>
                  </svg>
                </div>

                <div className="absolute top-2 right-2 bg-slate-950/80 border border-zinc-800 rounded-lg px-2 py-1 flex items-center gap-1">
                  <Sun className="w-3.5 h-3.5 text-amber-500 animate-spin-slow" />
                  <span className="text-[9px] font-black text-white">הדמיית קרינה</span>
                </div>
              </div>

              {/* Text explanations and surface selectors */}
              <div className="w-full md:w-1/2 flex flex-col justify-between h-full space-y-3">
                <div className="space-y-1">
                  <h4 className="text-xs font-black text-white">בחרו סוג משטח:</h4>
                  <div className="grid grid-cols-3 gap-1.5">
                    {Object.keys(surfaces).map(s => (
                      <button
                        key={s}
                        onClick={() => setSelectedSurface(s)}
                        className={`py-1.5 rounded-lg text-[9px] font-bold cursor-pointer border transition-all text-center ${
                          selectedSurface === s 
                ...
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 text-right">
                  <span className="text-[9px] text-earth font-black block mb-0.5">כיצד המשטח משפיע:</span>
                  <p className="text-[11px] text-white leading-normal">{surfaces[selectedSurface].desc}</p>
                </div>

                {/* Feedback Loop Feature */}
                <div className="flex items-center justify-between bg-teal-950/15 border border-teal-500/20 rounded-xl p-2.5">
                  <div>
                    <span className="text-[9px] text-teal-400 font-black block">לולאת משוב קרחונים 🔄</span>
        ...
        {activeTab === 1 && (
          <div className="w-full h-full flex flex-col justify-between gap-4">
            ...
            <div className="grid grid-cols-2 gap-4 shrink-0">
              {/* Area A */}
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 space-y-2 text-right">
                <span className="text-[10px] font-black text-sky-400 block">אזור א' (Area A)</span>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-text-muted">טמפרטורה:</span>
                  <span className="text-xs font-black text-white">{tempA}°C</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                ...
                <div className="flex justify-between items-center text-[9px] text-text-muted pt-1">
                  ...
                  <span className="font-extrabold text-white">{pressA} hPa ({pressA > 1013 ? "רמה" : "שקע"})</span>
                </div>
              </div>

              {/* Area B */}
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 space-y-2 text-right">
                <span className="text-[10px] font-black text-purple-400 block">אזור ב' (Area B)</span>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-text-muted">טמפרטורה:</span>
                  <span className="text-xs font-black text-white">{tempB}°C</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                ...
                <div className="flex justify-between items-center text-[9px] text-text-muted pt-1">
                  <span>לחץ ברומטרי:</span>
                  <span className="font-extrabold text-white">{pressB} hPa ({pressB > 1013 ? "רמה" : "שקע"})</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-950 border border-zinc-800/80 rounded-2xl p-4 flex-1 flex flex-col justify-between min-h-[180px] relative">
              ...
              <div className="absolute top-2 right-2 bg-slate-950/80 border border-zinc-800 rounded-lg px-2 py-1 flex items-center gap-1">
                ...
                <span className="text-[9px] font-black text-white">הדמיית מפל לחצים</span>
              </div>
              ...
              <div className="flex-1 flex justify-between items-center relative overflow-hidden px-10">
                ...
                <div className="flex flex-col items-center space-y-1">
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center font-bold text-xs ${
                    pressA > 1013 
                      ? "border-sky-500/30 bg-sky-950/40 text-sky-400 animate-pulse" 
                      : "border-orange-500/30 bg-orange-950/40 text-orange-400"
                  }`}>
                    {pressA > 1013 ? "H" : "L"}
                  </div>
                  <span className="text-[9px] font-bold text-text-muted">אזור לחץ {pressA > 1013 ? "גבוה (רמה)" : "נמוך (שקע)"}</span>
                ...
                <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
                  {windDirection === "none" ? (
                    <span className="text-[10px] text-text-muted font-bold">אין רוח (לחצים שווים)</span>
                  ) : (
                    <div className="space-y-1.5 w-full flex flex-col items-center">
                      <span className="text-[10px] text-white font-black animate-pulse">
                      ...
                      <div className="w-full h-2 bg-zinc-900 border border-zinc-800 rounded-full overflow-hidden relative">
                        <div 
                          className={`absolute inset-y-0 w-8 bg-sky-400 rounded-full ${
                ...
                <div className="flex flex-col items-center space-y-1">
                  ...
                  <span className="text-[9px] font-bold text-text-muted">אזור לחץ {pressB > 1013 ? "גבוה (רמה)" : "נמוך (שקע)"}</span>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-2.5 text-center mt-2">
        ...
        {activeTab === 2 && (
          <div className="w-full h-full flex flex-col justify-between gap-4">
            ...
            <div className="bg-zinc-950 border border-zinc-800/80 rounded-2xl p-4 flex-1 flex flex-col md:flex-row gap-4 items-center min-h-[220px]">
              ...
              <div className="w-full md:w-1/2 bg-slate-900/60 border border-zinc-800 rounded-xl p-2 h-full min-h-[180px] flex items-center justify-center relative">
                ...
                <svg className="w-full h-full min-h-[160px]" viewBox="0 0 240 180">
                  ...
                  {/* Months text labels */}
                  {MONTHS.map((m, i) => (
                    <text key={i} x={37 + i * 15} y="172" fill="#71717a" textAnchor="middle" className="text-[6px] font-bold">
                      {m}
                    </text>
                  ))}

                  {/* Axes labels */}
                  <text x="25" y="15" fill="#ef4444" textAnchor="end" className="text-[6px] font-black">T (°C)</text>
                  <text x="215" y="15" fill="#38bdf8" textAnchor="start" className="text-[6px] font-black">P (mm)</text>
                </svg>

                {/* Score badge */}
                <div className="absolute top-2 right-2 bg-slate-950/80 border border-zinc-800 rounded-lg px-2 py-1 flex items-center gap-1">
                  <Gauge className="w-3.5 h-3.5 text-amber-500" />
                  <span className="text-[9px] font-black text-white">ניקוד: {score.correct}/{score.total}</span>
                </div>
              </div>

              {/* Game interaction board */}
              <div className="w-full md:w-1/2 flex flex-col justify-between h-full space-y-3 text-right">
                <div className="space-y-1">
                  <span className="text-[9px] text-earth font-black block">משימה לתלמיד:</span>
                  ...
                  {CLIMATE_ZONES.map(zone => {
                    ...
                    return (
                      <button
                        ...
                        className={`py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border text-center ${buttonClass}`}
                      >
                        {zone.name}
                      </button>
                    );
                ...
                <div className="flex gap-2">
                  {!checked ? (
                    <button
                      ...
                      className="flex-1 py-2 bg-earth hover:bg-teal-600 text-white font-bold text-xs rounded-xl cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed transition-all text-center"
                    >
                      בדיקת תשובה 🔍
                    </button>
                  ) : (
                    <button
                      ...
                      className="flex-1 py-2 bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs rounded-xl cursor-pointer transition-all text-center flex items-center justify-center gap-1.5"
                    >
            ...
            {checked && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-right animate-fade-in flex-shrink-0">
                <span className="text-[10px] text-earth font-black block mb-0.5">ניתוח אקלימי:</span>
        ...
        {activeTab === 3 && (
          <div className="w-full h-full flex flex-col justify-between gap-4">
            ...
            {/* Slider bar */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5 space-y-2 text-right shrink-0">
              <div className="flex justify-between items-center">
                <div>
                  <span className="text-xs font-black text-white">ריכוז פחמן דו-חמצני באטמוספרה:</span>
                  <span className="text-[9px] text-text-muted block">(מדד פחמן דו-חמצני באטמוספרה בחלקיקים למיליון)</span>
                </div>
                <span className="text-sm font-black text-amber-500">{co2Ppm} ppm</span>
              </div>
            ...
            <div className="bg-zinc-950 border border-zinc-800/80 rounded-2xl p-4 flex-1 flex flex-col md:flex-row gap-4 items-center min-h-[200px]">
              ...
              <div className="w-full md:w-1/2 bg-slate-900/60 border border-zinc-800 rounded-xl h-full min-h-[160px] flex items-center justify-center relative overflow-hidden">
                ...
                <div className="w-20 h-20 rounded-full bg-blue-900 border-2 border-sky-400/30 flex items-center justify-center relative">
                  <div className="w-16 h-16 rounded-full bg-emerald-800/35 blur-sm" />
                  
                  {/* Temperature readout inside Earth */}
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                    <span className="text-[7px] font-bold text-text-muted">חום עולמי:</span>
                    <span className="text-xs font-black text-white">{globalTemp}°C</span>
                  </div>
                </div>

                {/* Greenhouse blanket boundary line */}
                <div 
                  className={`absolute w-36 h-36 rounded-full border-2 border-dashed transition-all duration-300 ${
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 200 160">
                  ...
                  <path 
                    ...
                    fill="none" 
                    stroke="#ef4444" 
                    strokeWidth={isOptimal ? "1" : isHigh ? "2" : "3.5"} 
                    className="animate-pulse"
                  />
                  
                  <text x="145" y="145" fill="#f87171" className="text-[7px] font-black">
                ...
                <div className="absolute top-2 right-2 bg-slate-950/80 border border-zinc-800 rounded-lg px-2 py-1 flex items-center gap-1">
                  ...
                  <span className="text-[9px] font-black text-white">אפקט החממה</span>
                </div>
              </div>
              ...
              <div className="w-full md:w-1/2 flex flex-col justify-between h-full space-y-3 text-right">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3.5">
                  <span className="text-[9px] text-earth font-black block mb-0.5">ניתוח מצב אטמוספרי:</span>
                ...
                <div className="bg-teal-950/15 border border-teal-500/20 rounded-xl p-2.5 flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-teal-400 shrink-0" />
                  <p className="text-[10.5px] text-white leading-normal">
